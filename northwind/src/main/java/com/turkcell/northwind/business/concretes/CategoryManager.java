...
/*
 * 	ProductManager ile direk olarak aynı... 
 * 
 */
@Service
public class CategoryManager implements CategoryService {
	...
	public List<ListCategoryDto> getAll() {
		...
		List<ListCategoryDto> response = result.stream()
				.map(category -> this.modelMapperService.forDto().map(category, ListCategoryDto.class))
				.collect(Collectors.toList());
		return response;
	}

	@Override
	public void add(CreateCategoryRequest createCategorytRequest) {
		Category category = this.modelMapperService.forRequest().map(createCategorytRequest, Category.class);
